        ...
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed top-[50%] left-[50%] -translate-x-1/2 -translate-y-1/2 w-[90vw] md:w-[600px] max-h-[85vh] overflow-hidden bg-white dark:bg-[#1A1A1A] rounded-xl shadow-xl z-[51] border border-[#E5E5E5] dark:border-[#333333]">
            <div className="p-4 border-b border-[#E5E5E5] dark:border-[#333333] flex items-center justify-between">
              <Dialog.Title className="text-lg font-semibold text-color-primary dark:text-color-primary-dark">
                    ...
                    <div className="space-y-4 mb-4">
                      <div className="flex gap-2">
                        <input
                          ...
                          className="flex-1 px-4 py-2 rounded-lg bg-[#F5F5F5] dark:bg-[#252525] border border-[#E5E5E5] dark:border-[#333333] text-color-primary"
                        />
                        <button
                          onClick={() => setFilters({})}
...
function TabButton({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) {
  return (
    <button
      onClick={onClick}
      className={classNames(
        'inline-flex layer-transition  items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium  focus-visible:outline-none disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none  text-color-primary border border-1 border-light bg-background shadow-sm hover:bg-accent hover:-foreground h-8 rounded-md px-3 text-xs',
        active
          ? 'bg-blue-500/40 text-blue-500 dark:text-[#48AAFF]'
          : '',
      )}
    >
      {children}
    ...
    <div className="p-4 rounded-lg bg-bolt-elements-background-depth-1 border border-light hover:border-lightActive/70 transition-colors">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-medium text-color-primary dark:text-white">{repo.name}</h3>
        </div>
        <button
          onClick={onSelect}
