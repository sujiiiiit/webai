...
export const DialogTitle = memo(({ className, children, ...props }: RadixDialog.DialogTitleProps) => {
  return (
    <RadixDialog.Title
      className={classNames('text-lg font-medium text-color-primary flex items-center gap-2', className)}
      {...props}
    >
      {children}
    </RadixDialog.Title>
  ...
  return (
    <RadixDialog.Portal>
      ...
      <RadixDialog.Content asChild>
        <motion.div
          className={classNames(
            'fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white dark:bg-gray-950 rounded-lg shadow-xl border border-light z-[9999] w-[520px]',
            className,
          )}
          initial="closed"
          animate="open"
          ...
          <div className="py-4">
            ...
            >
              {items.length > 0 ? (
                ...
                <div className="text-center py-4 text-sm text-color-secondary">No items to display</div>
              )}
            </div>
          </div>
